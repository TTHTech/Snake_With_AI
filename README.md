...
**Game Snake cổ điển kết hợp 12 thuật toán Trí tuệ nhân tạo tự tìm đường, kèm chế độ nghiên cứu so sánh hiệu năng.**
...
- 🎮 **Chơi tay** hoặc để **12 thuật toán AI** tự chơi.
...
| **AI** | Chọn 1 trong 12 thuật toán cho rắn tự chơi |
...
| IDA* | ✅ | ✅ | ✅ | cao (lặp lại) | O(d) |
| **Safe A\*** | ✅ | ✅ | ➖ (ưu tiên sống) | O(V) | O(V) |
| **Q-Learning** | 🧠 tự học | ➖ | ➖ | O(1)/bước | O(states) |
...
### 11. Safe A* (A* an toàn, kiểu Hamiltonian) — *rắn gần như không chết* 🏆
**Ý tưởng:** không chỉ tìm đường ngắn nhất tới mồi mà còn **đảm bảo sống sót**. Đây là phiên bản thực dụng của chiến lược Hamiltonian, hoạt động cả khi có vật cản và từ vị trí xuất phát bất kỳ.
**Cách hoạt động (4 bước):**
1. Tìm đường **A\*** tới mồi.
2. **Mô phỏng** đi theo đường đó; sau khi ăn, kiểm tra đầu rắn **còn đường về tới đuôi không** (BFS). Nếu còn → an toàn, đi bước đầu.
3. Nếu không an toàn → **đi theo đuôi** để câu giờ, chờ lối thoát mở ra.
4. Nếu vẫn bí → chọn ô lân cận **còn nhiều không gian trống nhất** (flood fill) để sống lâu nhất.

**Ưu:** sống sót cực lâu, ăn được rất nhiều mồi (gấp hàng chục lần các thuật toán khác). **Nhược:** đường đi không phải lúc nào cũng ngắn nhất (đánh đổi để lấy an toàn); tốn tính toán hơn vì phải kiểm tra an toàn mỗi bước.

### 12. Q-Learning (Học tăng cường) — *rắn TỰ HỌC* 🧠
Khác hẳn 11 thuật toán trên (đều là tìm đường có sẵn): rắn **không được lập trình cách đi**, mà **tự học qua thử-sai**.
- **Trạng thái (11 bit):** nguy hiểm thẳng/phải/trái · hướng đang đi · mồi ở trái/phải/trên/dưới.
- **Hành động (3):** đi thẳng / rẽ phải / rẽ trái (tránh quay đầu 180°).
- **Phần thưởng:** ăn mồi `+10`, chết `−10`, lại gần mồi `+1`, ra xa `−1`.
- **Học (Q-table):** `Q(s,a) ← Q(s,a) + α·[r + γ·max Q(s',·) − Q(s,a)]`, chạy hàng nghìn ván với chiến lược **ε-greedy** (đầu tiên khám phá ngẫu nhiên, sau đó khai thác nước tốt nhất).

**Huấn luyện:** vào **AI → TRAIN-Q** (xem tiến trình ngay trong game, `Esc` để dừng & lưu). Bảng học lưu ở `qtable.json`; chọn **QLEARN** để xem rắn chơi bằng kiến thức đã học. *Dự án kèm sẵn bảng đã huấn luyện ~15.000 ván.*

**Ưu:** thể hiện rõ "máy học" — điểm tăng dần theo số ván tập. **Nhược:** do trạng thái rút gọn (không thấy toàn bàn) nên không tối ưu, dễ tự kẹt khi rắn dài; kém Safe A* về điểm tối đa.
...
2. Chọn **Level**, **Speed**, và **Runs** — số ván chạy mỗi thuật toán (1/5/10/20) để **lấy trung bình** (bấm để đổi vòng).
3. **START** → mỗi thuật toán chơi `Runs` ván **đến khi rắn chết** (hoặc đạt 1200 bước/ván).
   - `Space` tạm dừng · `↑/↓` đổi tốc độ xem · `Esc` bỏ qua thuật toán hiện tại.
4. Màn **REPORT** hiện bảng so sánh; bấm:
   - **Chart** → biểu đồ cột (Score / Steps / Nodes / Time) kèm **thanh sai số (độ lệch chuẩn)**, lưu `benchmark_chart.png`.
   - **Clear** → xóa dữ liệu · **Back** → quay lại.

Số liệu thu thập: **Score** (số mồi ăn được), **Steps** (số bước sống sót), **Nodes** (tổng số ô đã duyệt), **Time** (giây), **Nodes/Step** (chi phí trung bình mỗi bước). Khi `Runs > 1`, mỗi giá trị là **trung bình ± độ lệch chuẩn** qua các ván. Kết quả **tích lũy qua nhiều lần chạy** và tự xuất ra `benchmark_results.csv` để phân tích thêm.
...
├── Snake_SafeAStar.py      # Thuật toán Safe A* (gần như không chết)
├── Snake_QLearning.py      # Q-Learning (rắn tự học) + qtable.json
├── qtable.json             # Bảng Q đã huấn luyện sẵn
